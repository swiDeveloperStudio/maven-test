...
RUN echo "deb http://ftp.debian.org/debian/ jessie main" > /etc/apt/sources.list \
 ...
 && apt-get --quiet install --yes --no-install-recommends \
			make gcc g++ xmlindent tofrodos devscripts fakeroot build-essential debhelper mini-dinstall docker-engine python-jinja2 python-pkg-resources ninja-build zip \ 
 && apt-get clean
